...
A full-stack .NET 10 + React 19 + Next.js template combining ServiceStack backend with Next.js server with React Server Components. Uses ASP.NET Core Identity for auth, OrmLite for application data, and Entity Framework Core for Identity data management.
...
- Single container deployment with coordinated process management in [Dockerfile](./Dockerfile) and [entrypoint.sh](./entrypoint.sh)
